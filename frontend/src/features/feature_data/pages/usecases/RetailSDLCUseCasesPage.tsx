...
import RetailSDLCUseCasesSearch from '../../components/RetailSDLCUseCasesSearch';
...
const RetailSDLCUseCasesPage: React.FC = () => {
  const [page, setPage] = useState(1);
  const [pageSize] = useState(30);
  const [searchQuery, setSearchQuery] = useState('');
  const [thoughts, setThoughts] = useState<string[]>([]);
  const [currentThought, setCurrentThought] = useState<string>('');
  const { useCases, totalCount, isLoading, handleGenerateReport, isGenerating } = useSLDCReport({ 
    page, 
    pageSize,
    searchQuery 
  });
  ...
  return (
    <MainLayout>
      <div className="container-fluid">
        <BreadcrumbWidget
          mainTitle={`Retail SDLC Use Cases (${totalCount})`}
          breadcrumbs={[
            { title: "Dashboard", url: "/" },
            { title: "Reports" },
            { title: "Retail SDLC Use Cases" },
          ]}
        />
            ...
            <div className="card custom-card">
              <div className="card-header d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex align-items-center gap-2">
                  <RetailSDLCUseCasesSearch 
                    onSearch={handleSearch}
                  />
                  <ExportButtons endpointUrl="/content/sdlc-use-cases/" searchQuery={searchQuery} />
                </div>
              </div>
              ...
              <div className="card-body">
                ...
                {!isLoading && (!useCases || useCases.length === 0) && (
                  <div className="alert alert-info" role="alert">
                    No SDLC use cases found. Generate a report to get started.
                  </div>
                )}
                ...
                {useCases && useCases.length > 0 && (
                  <>
                    ...
                    <div className="table-responsive">
                        ...
                        <tbody>
                          {useCases.map((useCase) => (
                            <tr key={useCase.id}>
                              ...
                              {/* <td className="col-md-2">{useCase.industry_segment || 'N/A'}</td> */}
                              <td className="col-md-2">
                                {useCase.sdlc_tools.split(',').map((tool: string, index: number) => (
                              ...
                              <td className="col-md-4"><TruncatedDescriptionWidget description={useCase.metric_impact} maxLength={100} /></td>
                      ...
                      <PusherListener streamKey="retail_sdlc" onThoughtReceived={handleThoughtReceived} />
                        {currentThought && (
...
export default RetailSDLCUseCasesPage; 